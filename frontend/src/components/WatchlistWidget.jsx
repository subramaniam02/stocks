...
function WatchlistRow({ quote, onTickerClick, onRemove }) {
  ...
  return (
    <div className="flex items-center gap-2 px-3 py-1.5 hover:bg-slate-800/60 transition-colors group">
      <button onClick={() => onTickerClick?.(quote.ticker)} className="flex-1 min-w-0 text-left">
        <span className="text-xs font-semibold text-slate-200 truncate block">{quote.ticker}</span>
      </button>
      <span className="text-right shrink-0">
        <span className="block text-xs font-medium tabular-nums text-slate-300">{fmtPrice(quote.price)}</span>
      ...
      <button
        ...
        className="p-1 -m-1 rounded opacity-0 group-hover:opacity-100 hover:bg-slate-700/60 transition-opacity shrink-0"
      >
      ...
      {open && (
        <div ref={panelRef} className="fixed left-16 top-1/2 -translate-y-1/2 w-72 max-h-[85vh] bg-slate-900 border border-slate-700 rounded-xl shadow-2xl z-50 overflow-hidden flex flex-col">
          <div className="flex items-center justify-between px-3 py-2 border-b border-slate-700 bg-slate-800/40 shrink-0">
            <span className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">Watchlist</span>
            <div className="flex items-center gap-1">
              <button onClick={load} disabled={loading} className="p-1 -m-1 rounded hover:bg-slate-700/60 transition-colors disabled:opacity-40">
          ...
          <div className="overflow-y-auto flex-1 min-h-0">
            {quotes.length > 0 ? (
              ...
              <p className="text-xs text-slate-600 px-3 py-3">
